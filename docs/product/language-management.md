...
Russian (`ru`) is the default locale. A user has an independent locale preference exposed at `GET`/`PUT /api/v1/me/locale`; the authenticated OIDC subject, not a browser supplied identity, is the subject source. The current foundation intentionally supports `ru`, `en` and `de` as configured locales.
