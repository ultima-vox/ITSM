...
class ProblemQueryOrganizationTest {
  @AfterEach
  void clearSecurity() {
  ...
  void findByIdIncludesTrustedOrganizationPredicate() {
    ...
    SecurityContextHolder.getContext().setAuthentication(new JwtAuthenticationToken(
        Jwt.withTokenValue("token").header("alg", "none")
            .claim("sub", "operator").claim("organization_id", "org-red").build()
    ));

    new ProblemQuery(jdbc).findById(UUID.randomUUID());
    ...
    assertThat(args.getValue()).contains("org-red");
  }
}
